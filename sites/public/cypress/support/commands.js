...
Cypress.Commands.add("step1PrimaryApplicantName", (application, autofill) => {
  if (!autofill) {
...
Cypress.Commands.add("step2PrimaryApplicantAddresses", (application, autofill) => {
  if (!autofill) {
    ...
    cy.getByTestId("app-primary-address-street2").type(
      application.applicant.applicantAddress.street2
    )
...
Cypress.Commands.add("step7AddHouseholdMembers", (application, autofill) => {
  ...
  application.householdMember.forEach((householdMember) => {
    if (autofill) {
      cy.getByID(`edit-member-${householdMember.firstName}-${householdMember.lastName}`).click()
    } else {
...
Cypress.Commands.add("step8PreferredUnits", (application, autofill) => {
  if (!autofill) {
...
Cypress.Commands.add("step17Demographics", (application, autofill) => {
  if (!autofill) {
...
Cypress.Commands.add(
  "submitApplication",
  (listingName, application, signedIn, verify, autofill) => {
    if (signedIn) {
      cy.beginApplicationSignedIn(listingName, autofill)
    } else {
